...
class PCK(nn.Module):
    ...
    def forward(self, noisy_2d, gt, thr=0.5):
        # get heatmap coordinate(x,y)
        gt = gt.detach().cpu().numpy()
        noisy_2d = noisy_2d.detach().cpu().numpy()
        ...
        # for index, (joint2d, heatmap, scale, offset) in enumerate(zip(joint2ds, heatmaps, scales, offsets, flag)):
        ...
        # PCK_dist = np.linalg.norm(joint2d[j][:2] - gt[:,:,:2])
        PCK_dist = np.linalg.norm(noisy_2d - gt, axis=2)
        print(PCK_dist)
        
        dist_cal = np.not_equal(PCK_dist, -1)
        num_dist_cal = dist_cal.sum()
        if num_dist_cal > 0:
            pck = np.less(PCK_dist[dist_cal], thr).sum() * 1.0 / num_dist_cal
            print(np.less(PCK_dist[dist_cal], thr).sum())
            return pck
        else:
            return -1
...
class MPJPE2D(nn.Module):
    def __init__(self, device, dtype=torch.float32):
        super(MPJPE2D, self).__init__()
        self.dtype = dtype
        self.device = device
        self.L1Loss = nn.L1Loss()
        self.weight = 100.

    def forward(self, heatmaps, scales, offsets, gt, flag, img_path):
        # get heatmap coordinate(x,y)
        gt = gt.detach().cpu().numpy()
        heatmaps = heatmaps.detach().cpu().numpy()
        heatmaps = heatmaps.transpose(0, 2, 3, 1)
        ...
        joint2ds[:, :, -1] = confidence
        for index, (joint2d, heatmap, scale, offset) in enumerate(zip(joint2ds, heatmaps, scales, offsets)):
            for j in range(heatmap[0].shape[-1]):
                joint2d[j][:2] = np.mean(np.where(heatmap[:, :, j] == joint2d[j][2]), axis=1)[::-1]
                joint2d[j][:2] = joint2d[j][:2] - offset
                joint2d[j][:2] = joint2d[j][:2] / scale
        ...
        # vis
        from utils.dataset_handle import draw_pose
        from utils.imutils import vis_img
        for img, pred_kp2d, gt_kp2d in zip(img_path, joint2ds, gt):
            img = cv2.imread(img)
            img = draw_pose(img, pred_kp2d, (0,0,255))
            img = draw_pose(img, gt_kp2d, (0,255,255))
            vis_img('img', img)
            

        mpjpe2d = np.sqrt(np.sum((joint2ds[:,:,:2] - gt[:,:,:2])**2, axis=-1)).mean()

        return torch.tensor(mpjpe2d, dtype=torch.float32)
...
class POSE_L2(nn.Module):
    ...
    def forward(self, pred, gt, flag):
        loss = 0.
        loss += self.L2Loss(pred[flag==1], gt[flag==1])
        ...
        # loss += self.L2Loss(pred[4][flag==1], gt[3][flag==1])
...
class MASK_L2(nn.Module):
    ...
    def forward(self, pred, gt, flag):
        ...
        # pre (mask) predicted by the model, which is a tensor containing multiple channels
        loss = 0.
        loss += self.L2Loss(pred[0][flag==1], gt[0][flag==1])
        loss += self.L2Loss(pred[1][flag==1], gt[0][flag==1])
        loss += self.L2Loss(pred[2][flag==1], gt[1][flag==1])
