...
class PoseNet(nn.Module):
    def __init__(self):
        ...
        self.block5 = nn.Sequential(maskBasicBlock(546, 256),maskBasicBlock(256, 64))
        self.block6 = nn.Sequential(maskBasicBlock(273, 256),maskBasicBlock(256, 64))
        self.block7 = nn.Sequential(maskBasicBlock(145, 128),maskBasicBlock(128, 64))
        self.conv = nn.Conv2d(in_channels=64, out_channels=17, kernel_size=1)
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.softmax = nn.Softmax()

    def forward(self, x, heat):
        header = torch.cat([self.conv0_0(x),self.conv0_1(x),self.conv0_2(x)], 1) # concatenate
        ...
        stage0 = torch.cat([heat_map0, heat, block3], 1) # combining intermediate features
