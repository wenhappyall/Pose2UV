...
class TokenPose_TB_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False, apply_multi=True, hidden_heatmap_dim=64*6,heatmap_dim=64*48,heatmap_size=[64,48], channels = 3, dropout = 0., emb_dropout = 0., pos_embedding_type="learnable"):
        ...
        self.to_keypoint_token = nn.Identity() # initialize identity mapping

        self.mlp_head = nn.Sequential(          #initialize mlp_head to generate heatmap
            nn.LayerNorm(dim),
            nn.Linear(dim, hidden_heatmap_dim),
            nn.LayerNorm(hidden_heatmap_dim),
            nn.Linear(hidden_heatmap_dim, heatmap_dim)
        ...
        trunc_normal_(self.keypoint_token, std=.02)     # Truncated normal distribution to prevents the initialization from being too large or too small, avoiding the risk of exploding or vanishing gradients. 
        if apply_init:
            self.apply(self._init_weights)
    ...
    def forward(self, feature, mask = None):
        ...
        b, n, _ = x.shape # b=batch_size, n=num_pathc

        keypoint_tokens = repeat(self.keypoint_token, '() n d -> b n d', b = b)  # copy to get tensor [batch_size, num_keypoint, dim]
        if self.pos_embedding_type in ["sine","sine-full"] :
            x += self.pos_embedding[:, :n]
            x = torch.cat((keypoint_tokens, x), dim=1)
        else:
