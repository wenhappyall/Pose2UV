...
def init(note='MP', dtype=torch.float32, output='output', **kwargs):
    ...
    if not kwargs.get('eval'):
        logger.set_names(['Epoch', 'LR', 'Train Loss', 'Test Loss', 'AP', 'AP.5', 'AP.75', 'AR', 'AR.5', 'AR.75'])
    else:
...
class ModelLoader():
    def __init__(self, model=None, lr=0.0001, device=torch.device('cpu'), pretrain=False, pretrain_dir='', out_dir='', smpl=None, generator=None, pretrain_poseseg=False, uv_mask=None, test_loss='MPJPE',  **kwargs):
        ...
        if pretrain:
            ...
            premodel_dict = torch.load(pretrain_dir)
    ...
    def save_poseseg_results(self, results, iter, batchsize=10):
        ...
        imgs_new = results['imgs_new'].transpose(0, 2, 3, 1)
        imgs_original = results['imgs_original'].transpose(0, 2, 3, 1)
        ...
        for index, (img_original, img_new, pred_heat, gt_heat) in enumerate(zip(imgs_original, imgs_new, pred_heats, gt_heats)):
            img_new = img_new * 255
            img_original = img_original * 255
            resized_image = cv2.resize(img_new,
                                   (int(pred_heat.shape[1]), int(pred_heat.shape[0])))

            pred_heat = np.max(pred_heat, axis=2)
            pred_heat = convert_color(pred_heat*255)
            pred_heat = cv2.addWeighted(resized_image.astype(np.uint8), 0.5, pred_heat.astype(np.uint8),0.5,0)
            ...
            gt_heat = cv2.addWeighted(img_original.astype(np.uint8), 0.5, gt_heat.astype(np.uint8),0.5,0)
            ...
            cv2.imwrite(os.path.join(output, img_name), img_new)
...
class LossLoader():
    def __init__(self, smpl, train_loss='L1', test_loss='L1', generator=None, device=torch.device('cpu'), uv_mask=False, batchsize=1, **kwargs):
        ...
        for loss in self.test_loss_type:
            ...
            if loss == 'MPJPE2D':
                self.test_loss.update(MPJPE2D=MPJPE2D(self.device))
            if loss == 'MASK_L2':
    ...
    def calcul_trainloss(self, pred, data): #  training loss of the poseseg model.
        loss_dict = {} # use to store the calculation results of each loss type

        # squeeze the last dimension of data['uv_flag'] into a tensor without that dimension.
        ...
        #     pred['pred_uv'] = pred['pred_uv'] * self.uv_mask # The predicted UV coordinates (pred['pred_uv']) are multiplied with self.uv_mask .

        for ltype in self.train_loss: # This attribute(self.train_loss) defines the type of loss used during training.
            if ltype == 'w_L1':
            ...
            # calculate the L1 loss of the mask and add it to loss dict.
            elif ltype == 'MASK_L1':
            ...
            # calculate the L2 loss of the mask and add it to loss dict.
            elif ltype == 'MASK_L2':
                loss_dict.update(MASK_L2=self.train_loss['MASK_L2'](pred['premask'], data['mask'], data['mask_flag']))
            elif ltype == 'POSE_L2':
                loss_dict.update(POSE_L2=self.train_loss['POSE_L2'](pred['preheat'], data['fullheat6448'], data['pose_flag']))
            else:
                pass
        ...
        for k in loss_dict:
            ...
            loss += loss_temp
            # Update the value of loss_dict[k] to the rounded value of loss_temp (keeping three decimal places).
    ...
    def calcul_testloss(self, pred, data):
        ...
        for ltype in self.test_loss:
            if ltype == 'MPJPE':
                ...
                loss_dict.update(PCK=self.test_loss['PCK'](data['noisy_2d'], data['gt_kp2d']))
            elif ltype == 'MPJPE2D':
                loss_dict.update(MPJPE2D=self.test_loss['MPJPE2D'](pred['preheat'], data['scale'], data['offset'], data['gt_kp2d'], data['pose_flag'], data['img_path']))
            elif ltype == 'MASK_L2':
        ...
        for k in loss_dict: 
            loss += loss_dict[k]
            loss_dict[k] = round(float(loss_dict[k]), 3)
        return loss, loss_dict
