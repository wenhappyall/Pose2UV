...
def poseseg_test(model, loss_func, loader, viz=False, device=torch.device('cpu')):
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            # forward
            pred = model.model(data)
            preheat = pred['preheat']# preheat = pred['preheat'][4] shape = [15,17,256,256]
            ...
            if i < 1:
                results = {}
                results.update(imgs_new=data['img192256'].detach().cpu().numpy().astype(np.float32))
                results.update(imgs_original=data['img'].detach().cpu().numpy().astype(np.float32))
                results.update(pred_heats=pred['preheat'].detach().cpu().numpy().astype(np.float32))
