...
def create_poseseg(image, mask, lt, rb, input_2d, gt_2d, smpl, generator, img_path, occlusions=None, is_train=False):
    ...
    if is_train: # image preprocessing operations to increase the diversity of the training data.
        ...
        image, img_mask, input_2d, gt_2d, lt, rb, offset = croppad_kp(image, img_mask, input_2d, gt_2d, lt, rb, f=255, img_size=256, aug=True, return_trans=True)
        ...
        image, img_mask, input_2d, gt_2d, lt, rb, offset = croppad_kp(image, img_mask, input_2d, gt_2d, lt, rb, f=255, img_size=256, aug=False, return_trans=True)
        # noisy_2d = add_noise(original_kp2d, sigma)
    ...
    fullheat6448 = fullheatmap[0][:,:,8:-8]
    # img1 = draw_pose(dst_image, coco_kp, (0,0,255))
    # vis_img("img_input", img1)
    # tt = np.max(fullheat6448, axis=0)
    # gtt = convert_color(tt*255)
    # gtt_resize = cv2.resize(gtt, (192,256))
    # dst = cv2.addWeighted(gtt_resize,0.5, image192256.astype(np.uint8),0.5,0)
    ...
    ind = 0
    for kp in gt_2d:
        if max(int(kp[1]), int(kp[0])) > 255 or min(int(kp[1]), int(kp[0]))<0:  # img_size 255,check if out of range invalid
            ind += 1
            continue
    ...
    rgb_img = im_to_torch(dst_image)
    img_192256 = im_to_torch(image192256)
    ...
    data['img192256'] = img_192256
    data['fullheat6448'] = fullheat6448
