...
def heatmap_stand(width, height, sigma):
    ...
    for i in range(height):
        for j in range(width):
            # calculate the square distance between the current pixel (i, j) and the center of the image (width / 2, height / 2).
            # scaled by 1/sigma^2, sigma control the width of the Gaussian distribution,larger sigma results in a wider heatmap peak
            # 0.5: make the heatmap peaks more concentrated and avoid excessive diffusion.
            exponent = 0.5 * (pow(i - height / 2, 2.0) + pow(j - width / 2, 2.0)) * pow(1 / sigma, 2.0)
            if exponent > 4.6052: continue
            heatmap[i,j] = min(math.exp(-exponent), 1.0) # calculate the intensity value of the current pixel
...
def gen_heatmap(label, heatmap_st, label_format='coco'):
