...
def color_gamma_contrast(image):
    alpha = 1.0 + uniform(-0.5, 0.5) # adjust contrast.
    beta = (1.0 - alpha)*0.5 # adjust brightness.
...
def scale_kp(image, img_mask, input_2d, gt_2d, lt, rb, img_size=256, aug=False):
    original_kp = gt_2d.copy()
    if aug:
        s1 = random.uniform(0.5, 0.9) # scale range
    else:
        s1 = 0.7
    h, w = image.shape[:2]
    content_size = rb - lt # calculate the size of the bbox
    # To prevent wrong bbox
    if content_size.min() < 1 or content_size.max() > max(h, w):
        lt = np.array([0, 0], np.float32)
        rb = np.array([h, w], np.float32)
        ...
        ratio = 20 / content_size.min() 
        content_size = content_size * ratio # enlarge the content area minimum to 20
    # calculate the final scaling
    s = min((img_size-1) * s1 / content_size[0], (img_size-1) * s1 / content_size[1]) # content size[0] : width ,content size[1] :height 
    # fx: horizontal fy:vertical scaling ratios are both s. 
    image = cv2.resize(image, None, fx=s, fy=s, interpolation=cv2.INTER_LINEAR)
    img_mask = cv2.resize(img_mask, None, fx=s, fy=s, interpolation=cv2.INTER_LINEAR)
    # scale the keypoints
    input_2d[:, :2] = input_2d[:, :2] * s # get (x,y) * s
    gt_2d[:, :2] = gt_2d[:, :2] * s
    # img2 = draw_pose(image, input_2d, (0,0,255))
    # vis_img("img_scale1", img2)
    # img3 = draw_pose(image, gt_2d, (0,255,255))
    # vis_img("img_scale2", img3)
...
def add_noise(pose, sigma):
    gauss = np.random.normal(0, sigma, size=(13, 3))
    noisy_2d = pose + gauss
    return noisy_2d
...
def croppad_kp(image, mask, input_2d, gt_2d, lt, rb, f=255, img_size=256, aug=False, return_trans=False):
    ...
    center = (rb + lt) / 2
    content_size = rb - lt
    # center (output image)-center(cropped region) 
    offset = np.array([img_size/2-center[0], img_size/2-center[1]])
    if aug:
        pd = content_size.max() * 0.05 # adjust offset randomly for data enhancement
        offset[0] = offset[0] + random.uniform(-pd, pd)
        offset[1] = offset[1] + random.uniform(-pd, pd)
    ...
    offrb = rb + offset
    M = np.float32([[1, 0, offset[0]], [0, 1, offset[1]]])
    # affine transformation convert it into the output image size
    image = cv2.warpAffine(image, M, (img_size, img_size), flags=cv2.INTER_CUBIC,
                            borderMode=cv2.BORDER_CONSTANT,
                            borderValue=(0, 0, 0)) 
    mask = cv2.warpAffine(mask, M, (img_size, img_size), flags=cv2.INTER_CUBIC,
