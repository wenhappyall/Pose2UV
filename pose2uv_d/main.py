...
def main(**args):
    ...
    task = args.pop('task')  
    exec('from process import %s_train' %task) # use exec function to import the train function
    ...
    for epoch in range(num_epoch):
        ...
        if mode == 'train':
            ...
            # use eval function to proceed execute string expression，The previously dynamically imported training function is called
            eval('%s_train' %task)(model, loss, train_loader, epoch, num_epoch, viz=viz, device=device)       

            if (epoch) % 1 == 0:
                eval('%s_test' %task)(model, loss,test_loader,epoch, viz=viz, device=device)
            else:
                testing_loss = 9e10
        ...
        # logger.append([int(epoch + 1), lr, training_loss, testing_loss, mAP['AP'],  mAP['Ap .5'], mAP['AP .75'], mAP['AR'], mAP['AR .5'], mAP['AR .75']])
    ...
    #         if (epoch) % 1 == 0:
    #             testing_loss, mAP = eval('%s_test' %task)(model, loss,test_loader, viz=viz, device=device)
    #         else:
    #             testing_loss = 9e10
